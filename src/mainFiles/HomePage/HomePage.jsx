...
export default function HomePage() {
  ...
  return (
    <section className="bg-[#0A0908] scroll-smooth h-[101vh] w-full relative">
      ...
      <section
        id="Menu"
        className={`w-full h-[100vh] scroll-smooth z-50 p-0 m-0 backdrop-blur-md transition-all duration-50 fixed ${
...
>
  ...
  <div className="absolute inset-0 -z-10 overflow-hidden">
    <div className="absolute inset-0 bg-gradient-to-r from-[#8ACA4F] to-[#B6E89A] transition-transform duration-500 scale-y-0 group-hover:scale-y-100 origin-bottom transform" />
      ...
      {/* Header Section for Mobile Toggle (lg:hidden) */}
      <header className="scroll-smooth fixed top-0 left-0 z-60 flex h-20 w-full items-center justify-end px-4 sm:px-8 lg:hidden">
        <button
          className="outline-none transition-all duration-100 transform ease-in-out"
          onClick={() => setIsOpen(!isOpen)}
        >
          <svg
            className={`hover:cursor-pointer h-[40px] w-[42px] text-white font-bold transition-transform duration-200 ${
